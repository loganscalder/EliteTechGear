...
```{r plot_some_stuff}
density = ggplot(mergedData, aes(Profit)) + 
    geom_density(aes(fill = Keyword == "*"), alpha = .5)
density
profit_by_match_type = 
    ddply(mergedData,
          .var = "Match.Type",
          .fun = summarise,
          Cost = sum(Total_Spend),
          Profit = sum(Profit),
          Return = Profit/sum(Total_Spend),
          AvgProfitPerKeyword = Profit/length(Match.Type)
          )
...
profit_by_targeting = 
    ddply(mergedData,
          ...
          `AvgProfitPerClick($)` =round(`Profit($)`/Clicks, 2)
          )
```
